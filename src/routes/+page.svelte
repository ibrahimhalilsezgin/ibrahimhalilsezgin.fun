  ...
  const executeCommand = () => {
    ...
    if (input.trim()) {
      if (input === "erenkarayilan") {
        ...
        document.getElementById('eren')?.setAttribute(
          'src',
          'https://www.youtube.com/embed/HMMydN7Zv9o?autoplay=1&loop=1&start=15&playlist=HMMydN7Zv9o'
        );
        document.getElementById('eren')?.removeAttribute('hidden');
      }
...
<div class="grid grid-cols-[1fr_2fr_1fr] h-screen font-mono">
  <!-- Sol taraf -->
  <div class="flex items-center justify-center font-2xl border-r border-r-[#1a1a1ad5]"></div>

  <!-- Orta -->
  {#if show}
    <div
      transition:blur={{ duration: 1200, amount: 300, opacity: 0, easing: cubicOut }}
      class="flex justify-center font-2xl items-center"
    >
      <div>
        <div class="flex flex-col p-34"></div>
            ...
            <pre data-prefix="$">
              <code>
                <input
                  type="text"
                  bind:value={input}
                  class="border border-transparent focus:outline-none focus:ring-0 font-mono w-full"
                  id="command"
                  on:keypress={(e) => {
                    if (e.key == "Enter") return executeCommand();
                  }}
                />
              </code>
            </pre>
        ...
        <div class="mockup-browser border-base-300 border w-[600px] mx-auto">
  ...
  <!-- Sağ taraf (border-l fixli) -->
  <div class="flex items-center justify-center font-2xl border-l border-l-[#1a1a1ad5] h-full">
